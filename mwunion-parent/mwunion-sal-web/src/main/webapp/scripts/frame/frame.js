...
(function($) {
	...
	function initNavEvent() {
		...
		$("li.more-con").mouseover(function(){
			$("ul.m-nav-more").show();
		})
		$("li.more-con").mouseout(function(){
			$("ul.m-nav-more").hide();
		})
	}
					...
					this.content.children("li").each(function(){
						...
						$this.find("li").each(function(){
							$(this).click(function(e){
								if($(this).hasClass("active")){
									if($(this).children("a").hasClass("has_sub_menu") && $(this).children("ul").length > 0){
										$(this).removeClass("active");
										$(this).children("ul").hide();
									}
								}else{
	...
	function initScreen() {
		...
		Horn.Frame.screen={
				...
				close:function(id){
					...
					this.resize();
					var liObj = $("li[menuId="+id+"]");
					var active = liObj.attr("class");
				...
				init:function(){
					...
					this.zoom.click(function(){
						if($(this).hasClass("h-screen-tab-narrow")){
							...
							Horn.Frame.expandMenu();
							//Horn.Frame.expandHead();   //新版本2.0不需要再缩小头部   @wanglan
						}else{
							$(this).addClass("h-screen-tab-narrow");
							Horn.Frame.collapsMenu();
							//Horn.Frame.collapsHead();  //新版本2.0不需要再缩小头部   @wanglan
						}
						setSize();
						Horn.Frame.screen.resize();
					});
				...
				create:function(conf){
					if(Horn.Frame.screen.isLimit()){
						...
						//加动态计算宽度 start @wanglan
						var cont = this.content;
						var allWith = 0;
						$(">li",this.content).each(function( index ) {
							if(index != 0){
								var w = $("li:eq("+index+")",cont).outerWidth();
								allWith = allWith + w +2;
							}
						});
						this.content.css("width",(tabWidth+allWith)+"px");
						//加动态计算宽度 end @wanglan

						var iframeId = "mainFrame_"+conf.id;
						//渲染内容
						var tabBodyHtml="<iframe tabId=\""+conf.id+"\" name=\""+iframeId+"\" id=\""+iframeId+"\" style=\"display:none;\" frameborder=\"0\" width=\"100%\" height=\"100%\" scrolling=\"auto\" ></iframe>";
				...
				refreshTab:function(tabId){
					...
					var tab=Horn.Frame.screen.tabs[tabId];
					
					if(typeof tab !='object'){
						return;
					}
					var url=$(tab).attr("url");
					var iframeId = "jresplus_iframe_"+tabId;
					if(url!=""){
